 ...
 */
const addTicket = function(className, ticketDayOfWeek, evenOrOddWeeksOnly, ticketTime, parallelNumber, ticketName,
                           ticketTeacher, ticketPlaceGeneral, ticketPlaceSpecific, offsetTop, rowHeights)
{
    // check parameters
    const eventBaseWrapperStyle = createStyleFromTime(ticketTime, ticketDayOfWeek, offsetTop, rowHeights);
    ...
    if(evenOrOddWeeksOnly)
    {
        ...
        console.log(ticketHeaderDflex);
        ticketHeaderDflex.children[0].classList.add("ticket-week");
        ticketHeaderDflex.children[0].innerText = evenOrOddWeeksOnly == 1 ? "Lichý" : "Sudý";
        for(let attr of document.querySelector('.event-base-wrapper .ticket-header > .d-flex').attributes)
        {
            if(attr.name == "class") continue;
    ...
    // append ticket to the container and return it
    ...
    return eventBaseWrapper;
};

/**
 * 
 * @param {string} className css-compatible class name (unique if possible)
 * @param {string} borderColour colour for the left border of the ticket (as seen in the official schedules)
 * @param {string} backgroundColour background colour for the ticket
 */
